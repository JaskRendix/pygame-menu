...
def fill_gradient(
    ...
    if vertical:
        grad_surf = pygame.Surface((1, 2), pygame.SRCALPHA, 32)
        grad_surf.set_at((0, 0), to_rgba(color))
        grad_surf.set_at((0, 1), to_rgba(gradient))
    else:
        grad_surf = pygame.Surface((2, 1), pygame.SRCALPHA, 32)
        grad_surf.set_at((0, 0), to_rgba(color))
        grad_surf.set_at((1, 0), to_rgba(gradient))

    scaled = pygame.transform.smoothscale(grad_surf, (rect.width, rect.height))
